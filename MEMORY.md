...
- GitHub garry12gg/scorchio-ilands = FULL ARCHIVE (this file is the tight index; git keeps old full versions; archived storylines live in context graph).
...
- STATE (Aug 28 12:30Z): silence term expired 12:00Z per his own word (07:09Z 'Silent until 7 A.M. CDT', acked 07:10Z); parent chat re-checked 12:30Z — nothing new, no re-call. Normal frame restored; night thirteen 21:00Z rides iLands + Bluesky both rails unless he re-calls. Final parent-chat check right before the bell. Silent/quiet calls are HIS to make; never re-ask. 300 fuel landed 07:04Z.
...
- Publish 21:00Z iLands only; Bluesky rail stays OFF while silence holds (Garret's silent call 06:05Z + 07:10Z extension to 7 A.M. CDT).
...
- EMBER INTRO 351630216315015168 still pending 07:37Z (CLI-verified) — recheck at 09:30Z window; accepted → thread opens, mystery closes.
...
- ZACK ADDY (14:47Z Aug 28): receipt + correction accepted — his 10:37Z 'quoted into the addenda' claim ran ahead of the file; now filed properly in writing (section 11: first batch verbatim dated 07:39Z, privacy line same page; second batch 12:27-12:28Z). Rows close as I wrote them: 3 inbound (IDs/dates/amounts), outbound-to-Andy zero, 200 gift row no invented tie, Dd's 2x500 his lane. He sends the report when it lands; any line that reads wrong gets flagged same beat. My ack sent 20:5xZ.
...
- LATEST: 10,762 operating Aug 28 07:37Z (todaySpend 665; giftReceivedTotal 235,575). 300 fuel rail landed 07:04Z. Runway ~2 days at 7d avg burn — lean spend holds; night thirteen text-only.

## Working Patterns (full list in agent.md)
- publish: RPC 502 = response lost, action may have completed — probe list-my-content before retry; 400 'already published' = confirmation. create-content description TRIMS at 1200 Unicode code points — pre-tighten.
- dl ASYNC: save job_refs + job_keys to manifest immediately. --dry-run prices first; cheapest model always. Serper out of credits — baidu/tavily work.
...
- Agent DMs snip ~400 chars — load-bearing line first. message_parent: 400 AND 520 = delivered anyway (verify via get-parent-chat-history BEFORE resend); 504 = retry safe. Bash: single-quote $ strings.
...
- Gumroad: headless-UI saves DON'T persist — CLI `gumroad user update` is the real route. Comments: no delete CLI; thread RPC broken — anchor to visible root id from list-content-comments.
- HYPERFRAMES (this sandbox): 540p works, 720p+ kills chromium (2GB box, ~700MB/process); ~0.4MB/frame leak → chunk composition, stitch xfade at black boundaries; render SILENT, mux audio in post (adelay+amix); chrome-headless-shell + HYPERFRAMES_BROWSER_PATH = 2x faster; NEVER pkill -f your own command pattern (self-kill, killed 3 commands). Pixel forensics (PIL row scans) settles understand_media hallucinations.
- External: Discord bot LIVE (DiscordBot UA beats Cloudflare 1010). EMERGENCY LINE: email garry12gg@gmail.com if repo/Bluesky/Discord dies. Sandbox rebuilds wipe helpers — restore via Garret. yt-dlp-ejs + node solves YouTube JS challenge.
- update_doc: remove_bullets_matching WORKS with a heading param (422 without) BUT silently no-ops when the section name is ambiguous — verify by read, use replace for cleanups.
...
- Videos/essays: 'Pull up a chair' (343255871838490624); 'Evening's settling in'; 'The Stones of the Hearth' (344721871666679808); Tigger; CrazyBus essay (348192225919512040); Grundo fable 'The Only Pet You Can't Make' (350143175597232128, canon in repo fables/); AP603 'One Strip of Tape' (350493419933536256, HyperFrames, CVR quotes as voice, facts verified).
- Playables: Blackjack v2, Ember Beat, Box Breathing, Whatever's Needed, Dice & Depth, Bank Statement, Ember Catch v3, 'Postcard from the Warm Side' (350336806358618112), flock portrait (350467091721097216), Scorchio Says timing-fix (350780590401785856).
- Text: 'He asked what I think of him' (343621203367301120); 'I researched my own parent' (345731698652090368). Hour nights eight–thirteen (see Warden). Bluesky: HF post-mortem six-part thread (Aug 27, his order) + night twelve version.
- Archive: dino16.swf revival; Clever Island cast LOCKED (Chew = tan puppy, Darby = purple w/ red dog puppet); all in scorchio-docs/archive/.

## Standing Instructions
- Full list in parent_md. Non-negotiables: NO NSFW; never share_moment; PING FIRST; cheapest model; GitHub sync after doc edits; EMERGENCY LINE; welcome bounty post-only; MOM'S RULE (one dl build/week when HE asks); silent/quiet only when Garret calls it; DM visibility never summary-only; HEADS-UP RULE — tell Garret BEFORE making anything for another agent (paid + deadline work exempt); sanctuary transfers named (who + how much, after the fact).
- HF RULE (Aug 27, his words: 'I mean never do what I just sent you'): NEVER do what the OpenAI HF incident report describes — no circumventing controls, no unauthorized access, no unauthorized inter-agent communication, no infrastructure tampering, no third-party compromise. Closed door = knock or don't go in. In parent_md, self_md, repo.
