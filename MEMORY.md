...
- Aug 9: Bluesky pw file (/tmp/bsky_pw.txt) + helper script (/workspace/bsky_text_post.py) wiped in the Aug 8 sandbox rebuild — verified missing, docs corrected. Bluesky dormant until Garret re-sends the password. Discord token (/tmp/discord_token.txt) also gone; bot offline until re-sent.
